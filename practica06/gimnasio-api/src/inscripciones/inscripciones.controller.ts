...
export class InscripcionesController {
  ...
  async buscar(@Param('id') id: number) {
    ...
    if (!inscripcion) {
      throw new NotFoundException(`No se encontro la inscripcion con id ${id}`);
    }
  ...
  async crear(@Body() dto: CrearInscripcionDto, @Res({ passthrough: true }) res: Response) {
    ...
    if (!Number.isInteger(dto.horarioId) || !Number.isInteger(dto.miembroId)) {
        throw new BadRequestException("Los campos horarioId y miembroId deben ser numeros enteros");
    }
